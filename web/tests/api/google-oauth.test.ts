...
describe("GET /api/integrations/google-calendar/callback", () => {
  it("missing/mismatched state → 302 bounce to settings with invalid_state (CSRF)", async () => {
    ...
    expect(noCookie.headers.get("location")).toContain(
      "/agent/settings?integrations=google_calendar_error&reason=invalid_state"
    );
    ...
    expect(mismatched.headers.get("location")).toContain(
      "integrations=google_calendar_error&reason=invalid_state"
    );
  });
  ...
  it("provider error (user denied consent) → 302 bounce with the error reason", async () => {
    await createUser({ role: "agent", auth0_id: "auth0|a" });
    const req = new Request(
      "http://localhost/api/integrations/google-calendar/callback?error=access_denied&state=x"
    );
    const res = await callbackRoute(req);
    expect(res.status).toBe(302);
    expect(res.headers.get("location")).toContain(
      "integrations=google_calendar_error&reason=access_denied"
    );
  });
  ...
  it("token-exchange failure → 302 bounce, no token row written", async () => {
    ...
    setGoogleOAuthForTesting({
      exchangeCode: async () => {
        throw new Error("google exploded");
      },
    });
    ...
    expect(res.status).toBe(302);
    expect(res.headers.get("location")).toContain(
      "integrations=google_calendar_error&reason=exchange_failed"
    );
  ...
  it("happy path: exchanges code, upserts token row, redirects 302", async () => {
    ...
    expect(res.status).toBe(302);
    expect(res.headers.get("location")).toContain(
      "/agent/settings?integrations=google_calendar_connected"
    );
