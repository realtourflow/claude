...
import { GET as startRoute } from "@/app/api/me/integrations/microsoft-calendar/start/route";
import { GET as callbackRoute } from "@/app/api/integrations/microsoft-calendar/callback/route";
...
beforeAll(async () => {
  ...
  process.env.MICROSOFT_OAUTH_CLIENT_ID = CLIENT_ID;
...
afterAll(() => {
  // Don't leak the configured-Microsoft env into other test files.
...
afterEach(() => {
  setMicrosoftOAuthForTesting(undefined);
});
...
async function runStart(
  ...
  const req = new Request(
    "http://localhost/api/me/integrations/microsoft-calendar/start",
...
async function runCallback(opts: {
  ...
  const req = new Request(
    `http://localhost/api/integrations/microsoft-calendar/callback?code=${encodeURIComponent(
      opts.code
...
function fakeExchange(tokens: Partial<MicrosoftTokens>) {
  setMicrosoftOAuthForTesting({
    exchangeCode: async (): Promise<MicrosoftTokens> => ({
      access_token: "ms-access-default",
      refresh_token: "ms-refresh-default",
      expires_in: 3600,
...
describe("GET /api/me/integrations/microsoft-calendar/start", () => {
...
describe("GET /api/integrations/microsoft-calendar/callback", () => {
  ...
  it("happy path: exchanges code, upserts token row, redirects 302", async () => {
    ...
    expect(row?.access_token).toBe("ms-access-123");
    expect(row?.refresh_token).toBe("ms-refresh-123");
    expect(row?.account_email).toBe("agent@outlook.com");
  });

  it("reconnect: updates access_token but preserves the prior refresh_token when none is returned", async () => {
    const agent = await createUser({ role: "agent", auth0_id: "auth0|a" });
  ...
  it("after a successful connect, /api/me/integrations reports connected=true", async () => {
    await createUser({ role: "agent", auth0_id: "auth0|a" });
    fakeExchange({ account_email: "agent@outlook.com" });
    ...
    const body = (await res.json()) as {
      microsoft_calendar: { connected: boolean; account_email?: string };
    };
    expect(body.microsoft_calendar.connected).toBe(true);
    expect(body.microsoft_calendar.account_email).toBe("agent@outlook.com");
  });
});
